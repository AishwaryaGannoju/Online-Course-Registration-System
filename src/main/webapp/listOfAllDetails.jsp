...
<head>
    ...
    <title>List of all Details-Online Courses REG</title>
    <style>
        * {
            text-align: center;
            font-family: Cambria;
...
<div class="btns">	
 ...
 <table>
	<thead>
	<tr><th>Name</th><th>Email</th><th>Password</th><th>Course Id</th><th>Course Name</th></tr>
	</thead>
	<tbody>
		<c:forEach var="detail" items="${listOfAllDetails}">
			<tr>
				<td><c:out value="${detail.name}" /></td>
				<td><c:out value="${detail.email}" /></td>
				<td><c:out value="${detail.password}" /></td>
				<td><c:out value="${detail.cid}" /></td>
				<td><c:out value="${detail.cname}" /></td>
			</tr>
		</c:forEach>
	</tbody>
	</table>
